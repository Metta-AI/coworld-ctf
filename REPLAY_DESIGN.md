...
**One sentence:** a torch-lit stone DUNGEON-CRAWLER broadcast — cool grey-blue carved-stone arena with
localized warm ember/torch punctuation, red & blue the only saturated channel (team banners, relic
hearts, rune-lit pedestals), on a warm-dark stone stage, NES pixel type for every number, amber for
drama. Reference family: Game Art Partners "The Dungeon" top-down tileset / CraftPix top-down dungeon
packs / Dungeon Crawl Stone Soup — **cool stone STRUCTURE + warm torch DRAMA + saturated team
OBJECTIVES**, the professional top-down dungeon grammar.

**Board palette (from the reference family; cool stone dominant, warm accent localized):** flagstone
face `#8a9199`, carved-stone base `#6b7580` (dominant/recessive structure), mortar/seam shadow `#4a4f56`;
torch/ember accent `#e89b4d` (pools ONLY at glows — braziers, rune channels, heart halos — never a
scene-wide wash); team vermillion `#d24a52` / team cerulean `#4f97b3` (the ONLY fully saturated channel:
banners, relic hearts, pedestal rune-glow).

**Chrome palette (Ink & Print; warm, never pure #000/#fff):** stage/letterbox warm stone-shadow
`#16110d` → `#241a12` (NOT the old muddy dungeon-purple `#211826`/`#2e222f` — reverted); parchment/paper
`#ece3cf` for labels; structural ink `#2a1f16` (the brand line — all strokes/shadows warm-dark, L31);
team vermillion `#d24a52` / cerulean `#4f97b3`; drama amber `#f4b154`; carved-stone hairline/ghost
`#9a8b7e`. Framing: the arena canvas full-bleed center-stage in a warm torchlight pool (L32 — focus, not
a literal set; the torch-flicker luminance runs at a WHISPER, ~0.05 opacity, not the 0.5 haze that muddied
the board); chrome floats in reserved bands. Type: `nes-pixel.ttf` (in repo, `data/atlas/nes-pixel.ttf`,
vendored as data-URI) for display + numbers; system sans only for fine print. Register: **broadcast
scorebug** over the dungeon board (competition/arena, L23) — parchment ink is the COLOR metaphor, but the
FORM stays clean broadcast (typography + weight + whitespace, no wood-grain/parchment texture fills, no
bordered card grids).
...
ART_PROVENANCE: mixed · **COUNT: 22 wired board-art subjects — 7 raster masters (`data/hd/*.png`:
floor, wall, crew_red, heart_red/blue, pedestal_red/blue) + ~15 procedural HD sprite subjects drawn in
`hd.nim` (grenade, multi-stage blast, crosshair, 6 transport icons, HP pips, sound ring, impact ring,
tracer dot, aim dot, splatter, scoreboard panels, UI text) — plus 16 per-team crew rotations (1 master
× 16 aim steps).**

> **Current-state correction (art-recovery pass, this session).** The shipping renderer is the HD
> TILE path in `src/ctf/hd.nim`, NOT the procedural-wall/`arena_floor.png` scheme the prose below still
> describes. The wired masters are the seven `data/hd/*.png`: `floor.png`, `wall.png` (seamless tiles),
> `crew_red.png` (recolored per team), `heart_red/blue.png`, `pedestal_red/blue.png`. The soldier and
> the hearts in that set are the PROVEN-GOOD COMMITTED art — a clean top-down armored soldier and a clean
> video-game heart — and are NOT to be regenerated (a wholesale nanobanana regen degraded them into a
> "circle-medallion" crew and a glitchy-crystal heart; reverted via `git checkout HEAD`). Only the two
> ENVIRONMENT tiles were re-arted this pass, to move the board from a sci-fi bolted-metal deck to a
> torch-lit dungeon: `floor.png` = one smooth continuous weathered stone (no visible tile grid), `wall.png`
> = beveled raised carved-stone blocks with deep mortar shadow (dimensional, not a flat slab). Generated by
> `scripts/art/gen_hd.mjs only=floor,wall`, packed by `scripts/art/pack_hd.py` (env-tiles-only PLAN),
> verified by looking at both the master and the composited board before commit.
