...
proc hunterTune(): CombatTune =
  ...
  # arcStandoff (2026-07-29): the MOVEMENT companion to counterArc — back off a DISARMED
  ...
  # inert by design. A/B: SHIPBASE=1 COUNTERARC=1 ARCSTANDOFF=1 vs SHIPBASE=1 COUNTERARC=1.
...
proc runEpisode(seed, maxTicks, numPlayers: int, hunterSlots: seq[int]):
  ...
  # ARCFOE=1 (2026-07-29): force the CONTROL team to field an ARMED plasma-arc carrier —
...
proc main() =
  ...
  when defined(commsprobe):
    ...
    echo &"    PICK (v27 man-advantage): localEdge {csPick} -> EMIT {csPickEmit} -> " &
      &"HEARD-ONLY-ARM {csPickArm}"
    echo &"      (HEARD-ONLY-ARM>0 => a bot pressed/committed on a numbers edge it could NOT see " &
      &"itself — the fog-crossing payoff. EMIT=0 with localEdge>0 => ScPick is always crowded out " &
      &"of the one shout slot by a higher-priority scenario.)"
