## medSee (plan #16) MECHANISM PROBE — grabprobe.nim plus a FINAL -d:msprobe
## summary. A copy rather than an edit because the tracked grabprobe.nim is the
## byte-identical not-blind gate; and per the woundedBank lesson a probe counter
## must dump ONCE AT EXIT, never on a modulo of a perception-gated frame count.
##
## Usage (same seeds both sides; MEDSEE arms the lever per RUN, not per team):
##   nim c -d:release -d:msprobe --opt:speed -o:/tmp/ms_probe.out \
##     players/baseline/eval/mssum.nim
##   EVAL_MAP=gen EVAL_TEAMS=2 MEDSEE=1 /tmp/ms_probe.out --games 12 --seed 100
##
...
when isMainModule:
  ...
  when defined(msprobe):
    echo "=== MSPROBE MECHANISM SUMMARY (plan #16 3.1) ==="
    echo "medSeeOn=", (getEnv("MEDSEE").len > 0)
    echo "msScan=", msScan, "  msVisAny=", msVisAny,
      "  msVisNear=", msVisNear, "  msVisOffSpot=", msVisOffSpot
    echo "offSpotAvailPct=",
      (if msScan > 0: 100.0*msVisOffSpot.float/msScan.float else: 0.0)
    echo "msFire=", msFire, "  msPickSpot=", msPickSpot,
      "  msPickVis=", msPickVis, "  msPickVisOff=", msPickVisOff
    echo "pickVisOffPctOfFire=",
      (if msFire > 0: 100.0*msPickVisOff.float/msFire.float else: 0.0)
    echo "msTgtSeq=", msTgtSeq, "  msTgtJump=", msTgtJump, "  jumpPct=",
      (if msTgtSeq > 0: 100.0*msTgtJump.float/msTgtSeq.float else: 0.0)
    echo "msHeals=", msHeals, "  msWoundedFrames=", msWoundedFrames
    echo "healsPer10kWounded=",
      (if msWoundedFrames > 0: 10000.0*msHeals.float/msWoundedFrames.float else: 0.0)
