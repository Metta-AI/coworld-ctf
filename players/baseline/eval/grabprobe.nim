...
proc newDriver(slot, team, episodeSeed: int): Driver =
  ...
  # ⭐⭐ ffa4 CONTACT-VOLUME TEAM ISOLATION (2026-08-18, the engage-range cap).
  # Identical trap and identical shape to FFAMEDTEAM/FFAPEELTEAM above: all 16
  # bots share ONE process env, so a bare FFARANGE=<px> arms every team and the
  # "A/B" is a MIRROR that can only measure zero. FFARANGETEAM=<n> keeps the cap
  # on engine team index n and STRIPS it from the other three, which is also the
  # closest local analogue of the hosted field, where only we carry the lever and
  # three rivals do not. Raw team INDEX, not red/blue: `t` collapses every
  # non-zero team to Blue on a >2-team board, so red/blue would arm two of four.
  # FFARANGETEAM=9 (any out-of-range index) is the all-control baseline and is
  # exactly how the control arm of this A/B is run — same binary, same env shape,
  # so the two arms differ in ONE integer.
  when compiles(tune.ffaEngageCap):
    let ffaRangeTeam = getEnv("FFARANGETEAM")
    if ffaRangeTeam.len > 0 and team != parseInt(ffaRangeTeam):
      tune.ffaEngageCap = 0.0
  when compiles(tune.ffaHoldVoronoi):
    let ffaHoldTeam = getEnv("FFAHOLDTEAM")
    if ffaHoldTeam.len > 0:
      tune.ffaHoldVoronoi = team == parseInt(ffaHoldTeam)
...
proc main() =
  ...
  for g in 0 ..< games:
    ...
    when defined(rangeprobe):
      # ⭐⭐ DID THE MECHANISM FIRE? Printed per episode so a killed run is still
      # usable, and so a vacuous arm (rpBound == 0) is visible immediately rather
      # than after the whole sweep. See the counter decls in baseline.nim.
      echo &"RPEP seed={epSeed} armTeam={getEnv(\"FFARANGETEAM\")} " &
        &"cap={getEnv(\"FFARANGE\")} frames={rpFrames} bound={rpBound} " &
        &"cutMeanPx={(if rpBound > 0: rpCutSum / rpBound.float else: 0.0):.1f} " &
        &"hlReach={rpHlReach} hlDeep={rpHlDeep} hlFire={rpHlFire} " &
        &"| FUTILITY sel={rpSel} far={rpSelFar} " &
        &"farPct={(if rpSel > 0: 100.0 * rpSelFar.float / rpSel.float else: 0.0):.1f} " &
        &"meanPickPx={(if rpSel > 0: rpSelDSum / rpSel.float else: 0.0):.0f}"
      flushFile(stdout)
      rpFrames = 0; rpBound = 0; rpCutSum = 0.0
      rpHlReach = 0; rpHlDeep = 0; rpHlFire = 0
    when defined(lifeprobe):
