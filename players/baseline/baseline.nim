...
proc decide(bot: Bot, client: ProtocolClient): uint8 =
  ...
  for i in 0 ..< bot.enemies.len:
    ...
    when defined(shieldShotFresh):
      # shieldShot v2 tightening on the main-lineage shield-fire port: while
      # shielded (slow 3x gun), engage only tracks seen within
      # ShieldShotFreshTicks. The 110 px cap (maxEngage = CarrierFireRange)
      # tests the EXTRAPOLATED position; a stale track can pass it while the
      # real enemy is 600+ px away — that stale-shot class was 32% of
      # shielded shots vs swarm and carried 3 of the 4 friendly kills and
      # the 13% FF band beyond 160 px (task 1216952408581705 cell-B decode).
      if hasShield and not hasPlasma and
