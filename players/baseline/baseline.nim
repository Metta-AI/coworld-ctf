...
proc decide(bot: Bot, client: ProtocolClient): uint8 =
  ...
  when defined(mvprobe):
    inc mvFrames
    if alive: inc mvAlive
    if mvFrames mod 40 == 0:
      stderr.writeLine "MVPROBE slot=" & $bot.slot & " team=" & $bot.team &
        " frames=" & $mvFrames & " aliveFrames=" & $mvAlive &
        " myColor=" & myColor & " alive=" & $alive & " pos=" &
        (if alive: $int(me.x) & "," & $int(me.y) else: "-") &
        " tick=" & $bot.tick
  if not alive:
