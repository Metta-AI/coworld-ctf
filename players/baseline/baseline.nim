...
type
  ...
  CombatTune = object
    ...
    trenchCraft: bool         # ⭐ TRENCHCRAFT (2026-08-06): adapt to dug-pit cover now that
                              ...
                              #                of around it.
                              # Default ON; NOTRENCHCRAFT=1 disables for the A/B.
...
proc shippedCombatTune(): CombatTune =
  ...
  # ⭐ TRENCHCRAFT (2026-08-06): ships default ON, same class as anchorRelock/spraySingle
  # above — every gated branch is a structural no-op until TrenchMarks is non-empty (an
  # engine build that emits the label AND a map that rolled at least one pit), so turning
  # it on cannot regress a game that never sees a trench. NOTRENCHCRAFT=1 holds it off for
  # the A/B's control arm.
  result.trenchCraft = getEnv("NOTRENCHCRAFT").len == 0
