...
when defined(mateAimSpriteRead) or defined(walkinDodge):
  const
    ...
    SoldierRotSteps = 16      # src/ctf/sim.nim SoldierRotations
    SoldierRotBrads = AimBrads div SoldierRotSteps  # brads per baked step
  ...
  proc spriteAimBrads(client: ProtocolClient, at: Vec, color: string): int =
    ...
    for facingRight in [true, false]:
      ...
      for o in client.spriteObjectsWithLabel(label):
        ...
        if d < bestD:
          bestD = d
          result = floorMod(o.spriteId - SoldierSpriteBase, SoldierRotSteps) *
            SoldierRotBrads
...
proc decide(bot: Bot, client: ProtocolClient): uint8 =
  ...
  when defined(walkinDodge):
    ...
    #       than "not trained on THIS enemy",
