...
# ── ENGINE-COMPAT SHIM (glory-era showcase build) ───────────────────────────
# This bot is built from maxwell/paintbot-policy, whose engine emits labels
# the glory branch's engine does not (own aim / game params / endzone shapes).
# The consts are defined here with the SAME strings so the bot compiles; at
# runtime the scans simply return empty and the bot falls back to its
# pre-perception behaviour. Verified alive via the statue oracle (per-slot
# travel) before being trusted for the showcase recording.
const
