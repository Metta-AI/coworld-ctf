...
proc newDriver(slot, team, episodeSeed: int): Driver =
  ...
  if getEnv("GRABGATE") == "1":
    tune.grabGate = true
  # rallyWave per-team RE-STAMP (issue #20). shippedCombatTune() reads RALLY from the
  # process env, but the harness runs all 16 bots in ONE process — so without this the
  # "A/B" is a mirror. RALLYTEAM=red|blue gives ONE side the lever and re-stamps the
  # other back to plain v40, giving a deterministic, per-tick, seat-rotatable rig.
  let rallyTeam = getEnv("RALLYTEAM")
  if rallyTeam.len > 0:
    tune.rallyWave = (rallyTeam == "red" and t == Red) or
                     (rallyTeam == "blue" and t == Blue)
...
# ── GROUND-TRUTH SQUAD METRICS (M2/M3/M4), read from sim.players via slotTruth —
# never the bot's own fogged perception. Indexed [team]: 0 = Red, 1 = Blue.
var gtAlive: array[2, int]        # alive seat-samples
var gtPair120: array[2, int]      # ...in a MUTUAL nearest-neighbour pair within 120px
var gtNear120: array[2, int]      # ...with any mate within 120px
var gtNearSum: array[2, float]    # ...sum of nearest-mate distance
var gtContact: array[2, int]      # ...with >=1 living rival within 300px
var gtOutnum: array[2, int]       # ...and more rivals than mates within 300px
var gtAdvSum: array[2, float]     # ...sum of (mates - foes) within 300px
var gtTravel: array[2, float]     # M4: sum |dpos| over alive-to-alive samples
var gtTravelN: array[2, int]
...
proc main() =
  ...
  when defined(rwtruth):
    var prevX, prevY: array[16, float]
    var prevOk: array[16, bool]
  for g in 0 ..< games:
    let epSeed = seed + g
    ...
    when defined(rwtruth):
      for s in 0 ..< numPlayers:
        prevX[s] = 0.0; prevY[s] = 0.0; prevOk[s] = false
    var tick = 0
    while tick < ticks:
      for s in 0 ..< numPlayers:
      ...
      when defined(rwtruth):
        if tick mod 4 == 0:
          var tr: array[16, tuple[x, y: float, alive: bool, team: int]]
          for s in 0 ..< numPlayers: tr[s] = engine.slotTruth(s)
          for s in 0 ..< numPlayers:
            if not tr[s].alive: continue
            let tm = tr[s].team
            if tm < 0 or tm > 1: continue
            inc gtAlive[tm]
            if prevOk[s]:
              gtTravel[tm] += sqrt((tr[s].x - prevX[s]) * (tr[s].x - prevX[s]) +
                                   (tr[s].y - prevY[s]) * (tr[s].y - prevY[s]))
              inc gtTravelN[tm]
            var nearest = -1
            var nd = 1e18
            var mates300 = 0
            ...
            for j in 0 ..< numPlayers:
              if j == s or not tr[j].alive: continue
              let dd = sqrt((tr[s].x - tr[j].x) * (tr[s].x - tr[j].x) +
                            (tr[s].y - tr[j].y) * (tr[s].y - tr[j].y))
              if tr[j].team == tm:
                if dd < nd: nd = dd; nearest = j
                if dd <= 300.0: inc mates300
              else:
                if dd <= 300.0: inc foes300
            if nearest >= 0:
              gtNearSum[tm] += nd
              if nd <= 120.0:
                inc gtNear120[tm]
                # MUTUAL pair: my nearest mate's own nearest mate is me.
                var bn = -1
                var bd = 1e18
                for j in 0 ..< numPlayers:
                  if j == nearest or not tr[j].alive or tr[j].team != tm: continue
                  let dd = sqrt((tr[nearest].x - tr[j].x) * (tr[nearest].x - tr[j].x) +
                                (tr[nearest].y - tr[j].y) * (tr[nearest].y - tr[j].y))
                  if dd < bd: bd = dd; bn = j
                if bn == s: inc gtPair120[tm]
            if foes300 > 0:
              inc gtContact[tm]
              gtAdvSum[tm] += float(mates300 - foes300)
              if foes300 > mates300: inc gtOutnum[tm]
          for s in 0 ..< numPlayers:
            prevX[s] = tr[s].x; prevY[s] = tr[s].y; prevOk[s] = tr[s].alive
      let r = engine.result()
...
when isMainModule:
  ...
  when defined(rwprobe):
    echo "=== RWPROBE  M1 DIFFERENTIAL (plan #squad-1 §2 M1) ==="
    echo "rwFrames(plan-layer movement frames)=", rwFrames
    echo "rwElig=", rwElig, "  eligPct=",
      (if rwFrames > 0: 100.0 * rwElig.float / rwFrames.float else: 0.0)
    echo "rwTargetDiff=", rwTargetDiff, "  targetDiffPctOfElig=",
      (if rwElig > 0: 100.0 * rwTargetDiff.float / rwElig.float else: 0.0),
      "  meanShiftPx=", (if rwTargetDiff > 0: rwShiftSum / rwTargetDiff.float else: 0.0)
    echo "rwStepDiff=", rwStepDiff, "  *** M1 stepDiffPctOfElig=",
      (if rwElig > 0: 100.0 * rwStepDiff.float / rwElig.float else: 0.0),
      " ***  (REJECT < 15%)"
    echo "rwStepDiffSurv=", rwStepDiffSurv, "  stepDiffSurvPctOfElig=",
      (if rwElig > 0: 100.0 * rwStepDiffSurv.float / rwElig.float else: 0.0),
      "  (strictest: the step differs AND the target reached the nav emit)"
    echo "emit: rwSurvive=", rwSurvive, "  rwOverwritten=", rwOverwritten,
      "  rwEmitOther=", rwEmitOther, "  survivePctOfElig=",
      (if rwElig > 0: 100.0 * rwSurvive.float / rwElig.float else: 0.0)
    echo "gates: role=", rwGateRole, " carry/pocket=", rwGateCarry,
      " phase=", rwGatePhase, " depth=", rwGateDepth, " contact=", rwGateContact
  when defined(rwtruth):
    echo "=== RWTRUTH  GROUND-TRUTH SQUAD METRICS (M2/M3/M4) ==="
    for tm in 0 .. 1:
      let nm = (if tm == 0: "RED " else: "BLUE")
      let a = max(1, gtAlive[tm])
      let c = max(1, gtContact[tm])
      echo nm, " aliveSamples=", gtAlive[tm],
        "  M2 mutualPair120=", 100.0 * gtPair120[tm].float / a.float, "%",
        "  mateWithin120=", 100.0 * gtNear120[tm].float / a.float, "%",
        "  meanNearestMate=", gtNearSum[tm] / a.float, "px"
      echo nm, " contactFrames=", gtContact[tm],
        "  M3 outnumbered=", 100.0 * gtOutnum[tm].float / c.float, "%",
        "  meanLocalAdv=", gtAdvSum[tm] / c.float,
        "  M4 travelPerAliveFrame=",
        (if gtTravelN[tm] > 0: gtTravel[tm] / gtTravelN[tm].float else: 0.0)
