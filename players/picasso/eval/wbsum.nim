...
when isMainModule:
  ...
  when defined(wbprobe):
    echo "=== WBPROBE MECHANISM SUMMARY (plan #13 §3.1) ==="
    echo "hpReadFrames=", wbAllFrames
    echo "wbEntries=", wbEntries, "  wbFrames=", wbFrames,
      "  wbFinishSuspend=", wbFinishSuspend
    echo "wbLineSegs=", wbLineSegs, "  wbBreak60=", wbBreak60,
      "  breakRate=", (if wbLineSegs > 0: 100.0*wbBreak60.float/wbLineSegs.float else: 0.0)
    echo "wbBankDeaths=", wbBankDeaths, "  wbBankHeals=", wbBankHeals
    echo "hp1Segs=", wbHp1Segs, "  hp1Heals=", wbHp1Heals,
      "  hp1Deaths=", wbHp1Deaths, "  hp1Ticks=", wbHp1Ticks
    echo "hp1HealSharePct=", (if wbHp1Segs > 0: 100.0*wbHp1Heals.float/wbHp1Segs.float else: 0.0)
    echo "hp1MeanTicks=", (if wbHp1Segs > 0: wbHp1Ticks.float/wbHp1Segs.float else: 0.0)
