...
| `log` | `(level: i32, ptr: i32, len: i32) -> ()` | best-effort diagnostics, legal during `play_manifest`, `play_init`, `play_step`, and `play_retune`. `level` is an opaque signed `i32`; the host preserves it without filtering or assigning an enum. Accepted raw bytes appear in the local play harness as ordered lowercase `bytes_hex`. For live ladder entries (`init`, `step`, and `retune`), the server synchronously writes `FIRST_LIGHT_PLAY_LOG tick=T seat=S entry=ID phase=P level=N message="..."` to public/operator-visible stdout, with the message encoded by Nim `strutils.escape`; upload-time manifest-probe logs have no seat/entry identity and are not written there. The ABI admits one call of `MaxLogBytesPerCall` bytes per guest invocation and silently ignores later calls in every phase, including the upload-time manifest probe. The live sink separately admits four lines per seat per 24 ticks, suppresses the rest, and adds `dropped_previous=N` to the first admitted line of the next window. These diagnostics are not private or durable, never enter status, replay, game state, or `gameHash`, and synchronous stdout backpressure may delay the server loop. |
...
| `MaxLogCallsPerInvocation` / `MaxLogBytesPerCall` | 1 / 256 |
...
- Host-call budgets ruling (James, 2026-09-04):
  `MaxLogCallsPerInvocation` 4→1; `MaxEmitsPerStep` stays 2 so a play can retry
  once after a rejected emit, with the last accepted emission standing;
  `MaxSpatialCallsPerStep` stays 2 (one `nearest_cover` plus one
  `nearest_reachable`). The manifest probe now drops over-cap logs silently
