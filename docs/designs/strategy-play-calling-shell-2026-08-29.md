...
composes with the default-true Season 2 selector (`season2Shell`, section 3.2)
in one direction: a `"play"` slot with explicit `season2Shell: false` is a
validation error (`playSeatRequiresShell`), while the default configuration
with no `"play"` slot is legal and stays on the direct-input path. Goldens
cover an explicit-disabled all-input roster, an all-play roster, and a mixed roster,
