# Shell demo — retained 32-seat Season 2 integration smoke

This retained 32-seat (16 duos × 2) shell integration smoke was first run as
the 2026-08-31 "first light" milestone. It predates the complete Season 2
...
cover with time to spare. `SHELL_PLAY_*` log lines carry the upload/
...
- `SHELL_DEMO_PORT` — server port (default 21814).
- `SHELL_DEMO_MEASURE_ONLY=1` — run only the probe gates, no live server.
...
- `SHELL_MOVEMENT tick=... moving=N aiming=M` telemetry windows.
- The diagnostic lines below, all keyed by `tick=` so they join against each
  other, the `SHELL_INSTALL` stream, and the replay's per-tick masks.
...
Every line carries `tick=` (the same tick numbering as `SHELL_INSTALL`
...
- `SHELL_ANNOTATION ... kind=play_fault epoch=E entry=ID code=C
  ...
  `SHELL_INSTALL` line that follows on that tick and seat shows what
...
- `SHELL_PLAY_LOG tick=T seat=S entry=ID phase=P level=N
...
- `SHELL_PLAN_BUDGET tick=T seat=S revision=R visits=V units=U
...
- `SHELL_ZONE tick=T elapsed=E phases=N phase=P current=[x,y,w,h]
  ...
  when `SHELL_ZONE_LOG=1` for ticks 0–4 and every 60th tick. `elapsed`
  is the zone schedule's clock and remains 0 until the round starts.
- `SHELL_NAV tick=T pending_plans=N stale_path=[...] no_path=[...]` —
...
- `SHELL_COMBAT tick=T fired= aligning= none_shootable= vetoed=
...
- `SHELL_TIMING tick=T seats=N window_ticks=24 shell_us=...
...
script overlays `tests/fixtures/shell/shell_demo_config.json` onto
...
proof (`tests/test_shell_episode.nim` and the replay compat suite).
