# A Pre-Match Vote Phase: Policies Choose the Map and Mode
...
Restated: before an episode's players start playing, every play-seat
policy is shown a small, fixed ballot and casts one vote. The options are
3 pre-generated candidates plus a fourth, "random." The winner determines
everything about the episode that isn't per-seat strategy: map, mode
(Battle Royale, CTF, ...), team count, seats per team. "Random" is a
ballot line, not a synonym for "the engine does whatever it wants" —
section 5.
...
So the vote is meant to decide the thing `PlayContext` and the
episode-layer build already assume is decided before a lobby starts.
Three shapes, none ruled here:

1. **Pre-registration ballot** — a new phase ahead of today's lobby, so
   `PlayContext` stays honest the moment it's sent. Cheapest on the wire;
   most disruptive to the container-start order Shell §9.1 fixed (game up,
   health check, then players) precisely to avoid new choreography.
2. **Defer `PlayContext`'s gameplay payload until the vote closes** —
   seats register and chat immediately, but mode/map (and the section-10
   build) wait for the tally. Keeps container order untouched; breaks the
...
3. **Speculative build** — the engine builds all 3 candidates' episode
   ...
   No `PlayContext` timing change; real throwaway compute (Shell §3.1's
   per-episode build, up to 3x).

## 3. Ballot composition (our lane)

Three generated candidates plus "random," per Maxwell's spec.
...
BR_MAPGEN.md` §3.5) — and cannot be paid inside a thirty-second lobby. The
3 candidates must be **fresh instances of already fairness-certified map
families** (a new `genSeed` within a certified family, or 3 certified
families), re-run only through the cheap static checks at instantiation
— weld test, thumbnail test, anti-confetti, density-uniformity
(`BR_MAPGEN.md` §5.3, §4.7) — never the statistical certification itself.
This is the "guarantee, not filter" house rule: those static checks are
hard guarantees, so each of the 3 draws is unconditionally valid alone;
there is no best-of-3-for-quality step, because best-of-K only moves a
distribution's tail, never its support (`memory:
ctf-mapgen-guarantee-not-filter`). If the ballot ever spans modes, a CTF
candidate is drawn from CTF's own certified pool, not bent to match BR.
...
is a lobby a policy can attend. The 3 candidates — and whatever instance
"random" resolves to (section 5) — must exist, be validated, and be in
the episode's launch configuration before that health check passes,
regardless of which of section 2's three shapes James picks.

Candidates must clear the same gate a scheduled map does: nothing reaches
the ballot that would fail the checks gating today's declared variants
(e.g. BR's baked `genSeed: 1339` instance in `coworld_manifest_br.json`).
...
(`"vote_map_2"`), reusing `proposalId`/epoch/`callAccepted` verbatim.
...
- **Plurality wins**, including "random" as a candidate line.
- **Ties break uniformly at random** from the episode's own deterministic
  ...
  the same seed.
- **"Random" winning draws among the 3 certified candidates only** —
  never a fresh, uncertified roll — per section 7's constraint that the
  vote never selects outside what the certifier already blessed.
- **Abstention folds into the tally as an implicit "random" vote**
...
A vote's tally is not prose — it determines the map, mode, and roster the
rest of the episode simulates under, which argues for the second
precedent. That puts it with any other sim-config-determining change:
...
(`game_config`, `mapSpec`), never as an in-episode record. "Candidate 2
won" is a new *kind* of record — an in-episode input determining what was
...
specified, fixed set: paintbot's manifest declares `2v2`, `4ffa`, `4ffa8`,
`default`, `1v1`, `ctf-default`, `ctf-1v1`, `paintball`
(`coworld_manifest_paintbot.json`); BR's manifest declares one, `default`,
with a baked `mapSpec` (`genSeed: 1339`, `teams: 16`, `num_agents: 32`,
`coworld_manifest_br.json`). A vote cannot conjure a configuration outside
that set — it selects among declared variants, or declared instances
within one variant's map family. This is the load-bearing constraint:
**the certifier and scheduler define the space the vote may move inside;
the vote never enlarges it.**

**With a vote**, the scheduler declares a **ballot space** instead of one
variant — a small closed set of already-certified ids, or 3 instance
seeds within one already-fixed variant for the map-only case. Whether
that space can span modes and roster shapes (Maxwell's full ask) or only
map instances within one fixed roster (sidestepping section 2's
container-order problem) is section 9's lead question.

**Certifier scope is narrower today than a cross-mode ballot needs.** The
manifest's `certification` block is one cheap smoke fixture — paintbot's
is `lives: 1, hitPoints: 1, gameOverTicks: 1, maxTicks: 300`
(`coworld_manifest_paintbot.json:1925`), nothing like any scheduled
variant's real shape — proving only that a submitted container speaks the
protocol, not that it survives every mode it might land in. Proposal, not
a ruling: every mode/roster shape reachable by any ballot a policy is
entered into needs its own certification pass.
...
style policy votes for the open one. This is meta-strategy one level
above the play ladder — `docs/designs/BR_PLAYS.md` already frames "plays
are the strategy layer"; voting for the arena is the same idea applied to
setup instead of execution, and could become its own play later (a
`vote` play with `preferSize`/`preferMode` params) — named as a future
direction, not proposed for v1.
...
## 9. Open questions for James (and the scheduler/certifier owner)

1. **Placement and timing (§2).** Can the vote resolve before
   `PlayContext`'s mode/map fields are sent at registration and before the
   section-10 episode-layer build starts — or does one of those need to
   change, or does the engine build speculatively? The question everything
   else sits on.
2. **Wire shape (§4).** LobbyChat convention, dedicated opcode pair, or
   reused `PlayCall` — or a fourth shape.
3. **Replay record and hash membership (§6).** New opcode number; is the
   selection inside the gameplay hash chain (lifecycle precedent) or
   excluded like chat text?
4. **GameVersion claim.** Does a live vote claim a new GV the way
   glory-inc3 (GV48) and play-calling P2 (GV49) did, with its own
   fixture re-record?
5. **Scheduler contract.** Does "declare a ballot space" replace "declare
   one variant," and can it span modes/roster shapes in v1, or only map
   instances within one fixed variant?
6. **Certifier scope.** Does every reachable mode/roster shape need its
   own certification fixture, or does one fixture per manifest still
   stand?
7. **Speculative build cost.** Is parallel-building N candidates'
   episode layers acceptable per-episode compute, or does deferring
   `PlayContext` (§2 option 2) trade it for lobby latency instead?
8. **Ballot deadline vs. presence budget.** Does the vote get its own
   wall-clock window, and does an absent seat's non-vote count against it
   the way absence already counts toward `playSeatBindTicks` (Shell:2453)?
9. **Vote as a play.** Should casting a vote eventually be an eighth
   BR_PLAYS.md play, or stay a lobby-phase action outside the play/call
   vocabulary entirely?
