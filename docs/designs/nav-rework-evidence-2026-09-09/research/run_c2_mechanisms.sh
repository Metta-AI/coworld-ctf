...
run_dir="$HOME/nav-throughput-results-20260909/C2-mechanisms"
test ! -e "$run_dir"
mkdir -p "$run_dir"
cp src/shell/body_nav.nim "$run_dir/original.nim"
cmp "$run_dir/original.nim" "$HOME/c2-mechanisms-input/parent.nim"
...
cp "$HOME/c2-mechanisms-input/bench_body_danger_cache.nim" tools/
cp "$HOME/c2-mechanisms-input/bench_body_danger_trace.nim" tools/
...
sha256sum "$HOME/c2-mechanisms-input/"*.nim "$HOME/c2-mechanisms-input/traces/"*.txt > "$run_dir/input-hashes.txt"
for arm in parent candidate; do
 cp "$HOME/c2-mechanisms-input/$arm.nim" src/shell/body_nav.nim
...
for repeat in 1 2 3; do
 for arm in parent candidate; do
  ...
  for trace in "$HOME/c2-mechanisms-input/traces/"*.txt; do
