...
run_dir="$HOME/nav-throughput-results-20260909/C2"
test ! -e "$run_dir"
mkdir -p "$run_dir"
cp src/shell/body_nav.nim "$run_dir/original.nim"
cmp "$run_dir/original.nim" "$HOME/c2-input/parent.nim"
cp tools/bench_body_nav_rework.nim "$run_dir/original-bench.nim"
cmp "$run_dir/original-bench.nim" "$HOME/c2-input/parent-bench.nim"
...
sha256sum "$HOME/c2-input/"*.nim > "$run_dir/input-hashes.txt"
lscpu > "$run_dir/lscpu.txt"
for arm in parent candidate; do
 cp "$HOME/c2-input/$arm.nim" src/shell/body_nav.nim
 cp "$HOME/c2-input/$arm-bench.nim" tools/bench_body_nav_rework.nim
