...
proc replayVisibleCells(seat: BodyNavSeat, origin: BodyPoint, base: int) =
  ...
  var rowEnd = diameter
  var gridOffset = (origin.y - radius) * gridW + origin.x - radius
  for wordIndex in 0 ..< cache.words:
    var word = cache.bits[base + wordIndex]
    while word != 0:
      let kernelIndex = wordIndex * 64 + countTrailingZeroBits(word)
      # Set bits are visited in increasing order, including across empty words.
      while kernelIndex >= rowEnd:
        rowEnd += diameter
        gridOffset += gridW - diameter
      seat.danger.values[gridOffset + kernelIndex] +=
