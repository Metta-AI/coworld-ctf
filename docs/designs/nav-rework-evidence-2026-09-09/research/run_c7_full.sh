...
run_dir="$HOME/nav-throughput-results-20260909/C7-full"
test ! -e "$run_dir"
mkdir -p "$run_dir"
cp src/shell/body_nav.nim "$run_dir/original.nim"
cmp "$run_dir/original.nim" "$HOME/c7-full-input/base.nim"
...
cp "$HOME/c7-full-input/bench_body_danger_cache.nim" tools/
cp "$HOME/c7-full-input/bench_body_danger_trace.nim" tools/
...
sha256sum "$HOME/c7-full-input/"*.nim "$HOME/c7-full-input/traces/"*.txt > "$run_dir/input-hashes.txt"
for arm in parent candidate; do
 cp "$HOME/c7-full-input/$arm.nim" src/shell/body_nav.nim
...
for repeat in 1 2 3; do
 ...
 for arm in $arms; do
  ...
  for trace in "$HOME/c7-full-input/traces/"*.txt; do
...
cp "$HOME/c7-full-input/candidate.nim" src/shell/body_nav.nim
