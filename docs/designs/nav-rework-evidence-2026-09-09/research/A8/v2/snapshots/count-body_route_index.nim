...
type
  PixelRecurrenceStats = object
    calls, dequeues, distinctKeys, keyBytes: int64
    repeats, repeatedDequeues: array[3, int64]
    outcomes, outcomeDequeues: array[3, int64]
    mismatches, dequeueMismatches, weakRepeats, weakMismatches: int64
  PixelRecurrenceResult = tuple[path: PixelPath, dequeues: int64]

var pixelRecurrenceSeen: Table[string, PixelRecurrenceResult]
var pixelRecurrenceWeakSeen: Table[string, PixelPath]
var pixelRecurrenceStages: seq[PixelRecurrenceStats]
var pixelRecurrenceDequeues: int64
var pixelRecurrenceCalls: seq[tuple[stage: int, key, weakKey: string,
  category: int, dequeues: int64, targetCount: int]]
...
proc initPixelSearchScratch(index: BodyRouteIndex): PixelSearchScratch =
  ...
  pixelRecurrenceStages.add PixelRecurrenceStats()
...
proc pixelPathInBoxRaw(index: BodyRouteIndex, start, center: BodyPoint,
  ...
  while head < tail:
    ...
    for delta in NavNeighbors:
      ...
      if next.x < x0 or next.x > x1 or next.y < y0 or next.y > y1:
        continue
      let nextLocal = local(next)
      if not scratch.standable[nextLocal]:
        continue
      if exactEdges and delta.x != 0 and delta.y != 0:
        ...
        if not scratch.standable[local(sideX)] or
            not scratch.standable[local(sideY)]:
          continue
...
proc pixelPathInBox(index: BodyRouteIndex, start, center: BodyPoint,
    targets: openArray[int32], scratch: var PixelSearchScratch,
    exactEdges = false): PixelPath =
  # Diagnostic only: always execute the real search; never reuse a result.
  for target in targets:
    var decoded: BodyPoint
    if target >= 0:
      decoded = (target.int mod index.map.gridWidth * NavCell + NavCell div 2,
        target.int div index.map.gridWidth * NavCell + NavCell div 2)
    else:
      let encoded = -1'i64 - target.int64
      let width = (index.map.width + FineNavCell - 1) div FineNavCell
      let height = (index.map.height + FineNavCell - 1) div FineNavCell
      let phase = encoded div (width * height).int64
      let position = encoded mod (width * height).int64
      decoded = ((position mod width.int64).int * FineNavCell + phase.int mod FineNavCell,
        (position div width.int64).int * FineNavCell + phase.int div FineNavCell)
    doAssert decoded == index.routePoint(target)
  let weakKey = $start & ";" & $center & ";" & $exactEdges
  let key = weakKey & ";" & $targets
  let before = pixelRecurrenceDequeues
  result = index.pixelPathInBoxRaw(start, center, targets, scratch, exactEdges)
  let dequeues = pixelRecurrenceDequeues - before
  let category = if result.points.len > 0: 2 elif result.reachedTarget: 1 else: 0
  pixelRecurrenceCalls.add (pixelRecurrenceStages.high, key, weakKey,
    category, dequeues, targets.len)
  var counts = pixelRecurrenceStages[^1]
  inc counts.calls
  counts.dequeues += dequeues
  inc counts.outcomes[category]
  counts.outcomeDequeues[category] += dequeues
  if pixelRecurrenceSeen.hasKey(key):
    inc counts.repeats[category]
    counts.repeatedDequeues[category] += dequeues
    if pixelRecurrenceSeen[key].path != result: inc counts.mismatches
    if pixelRecurrenceSeen[key].dequeues != dequeues: inc counts.dequeueMismatches
  else:
    inc counts.distinctKeys
    counts.keyBytes += key.len.int64
    pixelRecurrenceSeen[key] = (result, dequeues)
  if pixelRecurrenceWeakSeen.hasKey(weakKey):
    inc counts.weakRepeats
    if pixelRecurrenceWeakSeen[weakKey] != result: inc counts.weakMismatches
  else:
    pixelRecurrenceWeakSeen[weakKey] = result
  pixelRecurrenceStages[^1] = counts
  ...
  for cellIndex in 0 ..< index.stats.navCells:
    ...
    for neighborIndex, delta in NavNeighbors:
      if (index.legalMoves[cellIndex] and uint8(1 shl neighborIndex)) == 0:
        continue
      let next = (cell.x + delta.x, cell.y + delta.y)
      if not index.map.legalNavMove(cell, next):
        raise newException(BodyMapError,
