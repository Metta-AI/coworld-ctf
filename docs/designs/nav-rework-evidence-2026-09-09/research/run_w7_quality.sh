...
test ! -e tools/investigate_body_ray_clearance.nim
run_dir="$HOME/nav-throughput-results-20260909/W7-repaired"
...
for file in src/shell/body_map.nim src/shell/body_safety_query.nim src/shell/body_hazard.nim src/shell/body_route_index.nim src/shell/body_nav.nim src/shell/body_route_query.nim tools/bench_body_nav_rework.nim tools/build_nav_route_corpus.nim; do
 ...
 cp "$HOME/w7-input/$(basename "$file")" "$file"
done
restore_sources() {
 for file in src/shell/body_map.nim src/shell/body_safety_query.nim src/shell/body_hazard.nim src/shell/body_route_index.nim src/shell/body_nav.nim src/shell/body_route_query.nim tools/bench_body_nav_rework.nim tools/build_nav_route_corpus.nim; do
...
restore_all() {
 restore_sources
 rm -f tools/investigate_body_ray_clearance.nim
}
trap restore_all EXIT
cp "$HOME/w7-input/investigate_body_ray_clearance.nim" tools/investigate_body_ray_clearance.nim
git diff > "$run_dir/source.patch"
sha256sum "$HOME/w7-input/"*.nim > "$run_dir/input-source-hashes.txt"
...
"$HOME/.nimby/nim-2.2.6/bin/nim" c -d:release -d:useMalloc -d:noSignalHandler --threads:on --opt:speed --stackTrace:on --nimcache:"$run_dir/cache-clearance" -o:"$run_dir/clearance-check" tools/investigate_body_ray_clearance.nim > "$run_dir/clearance-build.log" 2>&1
sha256sum "$run_dir/bench" "$run_dir/clearance-check" > "$run_dir/executable-hashes.txt"
set +e
taskset -c 5 "$run_dir/clearance-check" > "$run_dir/clearance.json" 2> "$run_dir/clearance.stderr"
clearance_exit=$?
echo "$clearance_exit" > "$run_dir/clearance.exit"
set -e
if [ "$clearance_exit" -ne 0 ]; then exit "$clearance_exit"; fi
set +e
...
taskset -c 5 "$run_dir/bench" --configured-tick > "$run_dir/configured.json" 2> "$run_dir/configured.stderr"
