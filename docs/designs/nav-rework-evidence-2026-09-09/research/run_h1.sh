...
run_dir="$HOME/nav-throughput-results-20260909/H1"
test ! -e "$run_dir"
mkdir -p "$run_dir"
cp src/shell/body_route_query.nim "$run_dir/original.nim"
cmp "$run_dir/original.nim" "$HOME/h1-input/parent.nim"
trap 'cp "$run_dir/original.nim" src/shell/body_route_query.nim' EXIT
...
sha256sum "$HOME/h1-input/"*.nim > "$run_dir/input-hashes.txt"
lscpu > "$run_dir/lscpu.txt"
for arm in parent candidate; do
 cp "$HOME/h1-input/$arm.nim" src/shell/body_route_query.nim
...
assert quality["route_hash"] == "ee2488d32085fb4de3457c4cf841298c14b87add2225964d934f80f2a7c1013d"
PYHASH
...
for arm in parent candidate; do
 set +e
 taskset -c 5 "$run_dir/bench-$arm" --latency > "$run_dir/$arm-latency.json" 2> "$run_dir/$arm-latency.stderr"
 echo "$?" > "$run_dir/$arm-latency.exit"
 set -e
done
