"""Evaluate preregistered C10 five-pair trace and regime limits."""
import json
import statistics
import sys
...
assert len(traces) == 45 and len(regimes) == 40
checks = []
...
q = quality["quality"]
quality_exact = quality_exact and q["scored_count"] == 3072 and q["missing"] == 0 and q["illegal"] == 0
quality_exact = quality_exact and len(q["strata"]) == 49 and all(
    row["missing"] == 0 and row["illegal"] == 0 and
    row["inflation_p95_pct"] <= .5 and row["inflation_max_pct"] <= 3
    for row in q["strata"])
