...
0.5 ms of close floor plus 0.2 ms of clear and scan. Correction per
`C5_ROOT_REVIEW.md` and the v2 attribution (`v2/C5_V2_REPORT.md`): those
144 rebuilds are exactly one first scheduled fill per seat, not recurring
work; "configured danger floor" was the wrong phrase for this workload.
Native values are root's to measure.

## 3. Recipe for root

`run_c5_profile.sh` (v1, superseded by `v2/run_c5_v2_profile.sh`): builds a
traced binary (markers on, counters off) and a counted binary (counters
on, trace off), runs `--configured-tick --pool-index 3` with each on the
pinned CPU, and hashes everything. Correction per `C5_ROOT_REVIEW.md`:
traced row timings include tracing overhead and are attribution only,
never a pair comparator; root's unprofiled C2/C3 comparisons remain the
authoritative timing. The v1 script also discarded run exit codes with
`|| true`; v2 records every exit code explicitly.
...
C5 PROFILE READY (v1; see v2/ for the current snapshot, recipe and attribution)
