...
run_dir="$HOME/nav-throughput-results-20260909/C8-mechanisms"
test ! -e "$run_dir"
mkdir -p "$run_dir"
cp src/shell/body_nav.nim "$run_dir/original.nim"
cmp "$run_dir/original.nim" "$HOME/c8-mechanisms-input/base.nim"
...
cp "$HOME/c8-mechanisms-input/bench_body_danger_cache.nim" tools/
cp "$HOME/c8-mechanisms-input/bench_body_danger_trace.nim" tools/
...
sha256sum "$HOME/c8-mechanisms-input/"*.nim "$HOME/c8-mechanisms-input/traces/"*.txt > "$run_dir/input-hashes.txt"
for arm in parent candidate; do
 cp "$HOME/c8-mechanisms-input/$arm.nim" src/shell/body_nav.nim
...
for repeat in 1 2 3; do
 ...
 for arm in $arms; do
  ...
  for trace in "$HOME/c8-mechanisms-input/traces/"*.txt; do
...
cp "$HOME/c8-mechanisms-input/candidate.nim" src/shell/body_nav.nim
