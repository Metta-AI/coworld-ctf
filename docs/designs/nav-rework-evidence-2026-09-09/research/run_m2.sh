...
run_dir="$HOME/nav-throughput-results-20260909/M2"
...
for file in src/shell/body_map.nim src/shell/body_hazard.nim src/shell/body_route_index.nim src/shell/body_nav.nim src/shell/body_route_query.nim tools/bench_body_nav_rework.nim tools/build_nav_route_corpus.nim; do
 cp "$file" "$run_dir/parent/$(basename "$file")"
 cp "$HOME/m2-input/$(basename "$file")" "$file"
done
restore_sources() {
 for file in src/shell/body_map.nim src/shell/body_hazard.nim src/shell/body_route_index.nim src/shell/body_nav.nim src/shell/body_route_query.nim tools/bench_body_nav_rework.nim tools/build_nav_route_corpus.nim; do
...
set +e
taskset -c 5 "$run_dir/bench" --activation > "$run_dir/activation.json" 2> "$run_dir/activation.stderr"
