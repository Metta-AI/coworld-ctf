...
SOURCE_ID = "i-045ddd6df51150967"
TARGET_NAME = "jamesboggs-nav-throughput-m8i"


def record(event, **fields):
    row = {"time": datetime.now(timezone.utc).isoformat(), "event": event, **fields}
    with (ROOT / "M8I_PROVISION.jsonl").open("a") as out:
...
def main():
    ...
    assert tags.get("Name") == "jamesboggs-nav-throughput-c8i"
    ...
    instance_id = launch_instance(
        ec2, name=TARGET_NAME, instance_type="m8i-flex.4xlarge",
    ...
    record("target_launched", instance_id=instance_id, instance_type="m8i-flex.4xlarge")
