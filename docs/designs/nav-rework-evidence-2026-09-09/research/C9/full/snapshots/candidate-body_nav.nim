...
proc materializeVisibleCells(seat: BodyNavSeat, origin: BodyPoint, slot: int) =
  ...
  ## each nonzero add as a list entry for later hits. One add per cell per source makes the list exact
  ## in any cell order. The cache is read through the seat field (no
  ## per-cell owner local).
  let
