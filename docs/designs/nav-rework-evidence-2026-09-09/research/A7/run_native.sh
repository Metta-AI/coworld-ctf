...
sha256sum "$HOME/a7-input/"*.nim > "$run_dir/input-hashes.txt"
for arm in eager eager-early lazy lazy-early; do
 cp "$HOME/a7-input/$arm.nim" src/shell/body_route_index.nim
...
for repeat in 1 2 3; do
 arms="eager eager-early lazy lazy-early"
 if [ "$repeat" = 2 ]; then arms="lazy-early lazy eager-early eager"; fi
 for arm in $arms; do
