...
const PixelStandabilityArm* {.strdefine.} = "eager"
static:
...
type
  ...
  PixelSearchScratch = object
    ...
    standable: seq[bool]
    when PixelStandabilityArm == "lazy":
...
proc initPixelSearchScratch(index: BodyRouteIndex): PixelSearchScratch =
  ...
  const generationArrays = if PixelStandabilityArm == "lazy": 3 else: 2
...
proc pixelPathInBox(index: BodyRouteIndex, start, center: BodyPoint,
  ...
  scratch.beginSearch()
  when PixelStandabilityArm == "eager":
    for y in y0 .. y1:
      for x in x0 .. x1:
        let point: BodyPoint = (x, y)
        scratch.standable[local(point)] = index.map.canStand(point)
    when defined(pixelStandabilityCounters):
      pixelStandability.eagerFills += boxW * boxH
      pixelStandability.canStandEvaluations += boxW * boxH
  ...
  template standableAt(localIndex: int, point: BodyPoint): bool =
    ...
    elif PixelStandabilityArm == "direct":
      when defined(pixelStandabilityCounters):
        inc pixelStandability.canStandEvaluations
      index.map.canStand(point)
    else:
