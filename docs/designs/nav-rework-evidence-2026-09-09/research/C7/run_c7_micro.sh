#!/usr/bin/env bash
# C7 native micro v2 (root runs, m8i CPU 5). Tools only; no production source
...
for round in 1 2 3 4 5; do
  # v2: reverse the binary order on alternating repeats (c2,c8 on odd rounds;
  # c8,c2 on even), results are keyed by arm in the JSON, not by order.
  if [ $((round % 2)) -eq 1 ]; then arms="c2 c8"; else arms="c8 c2"; fi
  for arm in $arms; do
    if [ "$arm" = c2 ]; then tree="$tree_c2"; else tree="$tree_c8"; fi
