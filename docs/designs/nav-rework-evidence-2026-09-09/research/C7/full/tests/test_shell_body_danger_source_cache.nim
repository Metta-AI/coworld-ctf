## Shared source-visibility cache (C2 policy, C7 list representation):
## exactness against the uncached ray walk, source order, close floor,
## bounded deterministic eviction, range identity, the retained ledger, and
## the list-specific risks: capacity-exact sources, stale entries after slot
## reuse, and index mapping at map borders across supported ranges.
...
suite "shared danger source visibility cache":
  ...
  test "entries are sized by the live range and owned per system":
    ...
    check near.dangerSourceCacheEntryBytes == 5_385 * 8
    check far.dangerSourceCacheEntryBytes == 54_173 * 8
    let a = (x: 40, y: 30)
  ...
  test "ledger counts the shared cache by capacity":
    ...
    check bytes.sharedDangerSourceCache >=
      int64(DangerSourceCacheSlots * nav.dangerSourceCacheEntryBytes)
    check bytes.sharedDangerSourceCache <
      int64(DangerSourceCacheSlots * nav.dangerSourceCacheEntryBytes) + 4096
...
suite "shared danger source list risks":
  test "an all-visible source fills a slot to exactly its capacity":
    ...
    nav.seats[0].rebuildDanger(map, input((8, 8), [center]), 1)
    check nav.dangerSourceCacheLengths == @[nav.dangerSourceCacheCapacity]
    ...
    nav.seats[0].rebuildDanger(map, input((8, 8), [center]), 2)   # hit, full-length replay
    check floatBytes(nav.seats[0].dangerSnapshot) ==
      floatBytes(reference.seats[0].dangerSnapshot)

  test "a reused slot replays only its new entries":
    ...
    nav.seats[0].rebuildDanger(map, input((8, 8), [long]), 1)
    let longLength = nav.dangerSourceCacheLengths[^1]
    for index in 0 ..< DangerSourceCacheSlots - 1:
    ...
    check nav.dangerSourceCacheOrder.len == DangerSourceCacheSlots
    nav.seats[0].rebuildDanger(map, input((8, 8), [short]), 101)     # hit on the reused slot
    let shortLength = nav.dangerSourceCacheLengths[^1]
    check shortLength < longLength
    check floatBytes(nav.seats[0].dangerSnapshot) ==
  ...
  test "border origins map every entry to the right cell across supported ranges":
    ...
    for range in [331, 1050, 1300, 1600]:
      ...
      for corner in corners:
        nav.seats[0].rebuildDanger(map, input((8, 8), [corner]), 1)   # miss (records)
        nav.seats[0].rebuildDanger(map, input((8, 8), [corner]), 2)   # hit (replays)
        check floatBytes(nav.seats[0].dangerSnapshot) ==
          floatBytes(rasterOf(map, [corner], range))
