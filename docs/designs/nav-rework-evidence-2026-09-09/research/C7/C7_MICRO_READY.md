...
## 0. v2 protocol fixes (per `C7_ROOT_MICRO_REVIEW.md`); v1 preserved under `C7/v1/`

- Arm order now alternates: bitmap then list on even batches, list then
  bitmap on odd batches, with results stored by arm (the JSON carries
  `batch_order` and `protocol: v2-alternating-precomputed-cells`). The
  native recipe reverses the c2/c8 binary order on alternating repeats.
- Origin cells are precomputed outside timing alongside bases and lists
  and used by both arms; `map.cellOf` no longer sits inside the bitmap's
  timed loop.
- All exactness checks and hashes are unchanged and still pass; the v2
  tool was rebuilt and smoked against both materialized C8 snapshots in
  place (`local-mac/snapbuild_*.log`) and the screen source restored and
  hash-verified. Hashes in `SHA256SUMS` are for the v2 tool and recipe.

Mac v2 results (informational; medians over three interleaved rounds, all
exact, origins identical across arms and rounds):
...
The v2 protocol changes nothing material on this host: list replay is
still about 8 percent of the C2 bitmap replay per source, and C8 alone
about 0.91 to 0.96 of C2. Section 3 below is the v1 table, kept as
history.
