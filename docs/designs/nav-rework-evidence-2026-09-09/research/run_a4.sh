...
test ! -e tools/check_body_index_validation.nim
run_dir="$HOME/nav-throughput-results-20260909/A4"
test ! -e "$run_dir"
mkdir -p "$run_dir"
cp src/shell/body_route_index.nim "$run_dir/original.nim"
cmp "$run_dir/original.nim" "$HOME/a4-input/parent.nim"
restore_sources() {
 cp "$run_dir/original.nim" src/shell/body_route_index.nim
 rm -f tools/check_body_index_validation.nim
}
trap restore_sources EXIT
cp "$HOME/a4-input/check_body_index_validation.nim" tools/check_body_index_validation.nim
...
sha256sum "$HOME/a4-input/"*.nim > "$run_dir/input-hashes.txt"
lscpu > "$run_dir/lscpu.txt"
for arm in parent candidate; do
 cp "$HOME/a4-input/$arm.nim" src/shell/body_route_index.nim
...
"$HOME/.nimby/nim-2.2.6/bin/nim" c -d:release -d:useMalloc -d:noSignalHandler --threads:on --path:src/shell -o:"$run_dir/check-validator" tools/check_body_index_validation.nim > "$run_dir/validator-build.log" 2>&1
"$run_dir/check-validator" > "$run_dir/validator-mutations.json"
