...
`battle-royale-s2`. **S8 changes SCORING only (GLORYVERSION 17→18) and NOT
the wire: GameVersion stays unchanged, and no fixture re-record is owed by
this ship.** Every constant this ship moves — old value to new value — is
in the table below; nothing not listed here changes.
