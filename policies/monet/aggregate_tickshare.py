#!/usr/bin/env python3
"""FOUR DIGITS lane (W1) -- aggregates the tick-share + track-presence
instrument (src/shell/ladder.nim, `-d:tickShareProbe`) across a run_series.sh
multi-seed series.
...
Reads, per seed directory OUT_ROOT/seed<i>/:
  ...
  - server.log          the probe's own `[tickshare] seat=.. tick=.. phase=..
                         native=0|1 trackHit=-1|0|1 distCenter=F distEdge=F`
                         lines, one per sampled (seat, tick) -- see ladder.nim's
                         module comment above tickShareProbeTick for exactly
                         what each field means and the ally-exclusion caveat
                         on trackHit.
    ...
    with log_path.open(errors="replace") as f:
        for line in f:
            ...
            rows.append(dict(
                ...
                native=int(m.group(4)), trackHit=int(m.group(5)),
