...
| harness retries a module upload that never reaches READY (`UPLOAD_RETRIES` 2) and drops a still-missing play from `available` so no later call is refused with `playUnknown` (hosted `manifestProbe` rejections hit ~1 in 5 seats; a rejected scatter/edge_ride left the seat with no ladder) | — | — | — | uploaded as `aaron-paintbot:v2` (haiku) / `v4` (qwen); XP `xreq_634889b9` (40 eps) + `xreq_a2b31386` (60 eps), 16 seats, v1+v2+v4 rotated vs top-13 fillers, 120 rows/arm: call rejections 6.17 -> 0.1-0.2 per episode, failures 28 -> 12-14, win rate (score >= 16) v1 31.7 / v2 32.5 / v4 26.7 % (±4) — no detectable Glory change; kept as a correctness fix, v1 stays champion |
| aggressive `adjust_entries`: drop every model `pact`, force `target_law.never=[]` and strip `holdTrigger`, `race` contested loot/supply, always carry a `loot` rung; persona notes say never-list is DO-NOT-SHOOT (the qwen seat put tracked enemies -- daveey, Lawrence, ... -- on it in 12% of solo episodes, once in a 2-alive endgame). `HUNTER_RIDE=wide` variant (margin 220-320) tried alongside | — | — | — | uploaded as `aaron-paintbot:v5` (+`v6` wide). 3-seat field XP `xreq_992dc142` (100 eps) showed nothing: with >1 entrant seat per lobby `ally_clones` pre-fills the never-list on every arm and the model stops adding enemies (v1 0.98 tags/ep there). Solo-seat XPs `xreq_c9baeb56` (v1, 83 clean) vs `xreq_530059bc` (v5, 100): tags/ep 0.59±0.10 → **1.11±0.11**, ≥1 tag 36 → 65%, Glory ≥16 13 → 29%, log1p Glory 1.76±0.16 → 2.52±0.19, survival unchanged (96% die), field 0.78 both runs, 0 tracebacks. v5 submitted as champion; v6 no signal. Caveat: v1 sat in the filler pool of every v5 episode (clone pact present). |
