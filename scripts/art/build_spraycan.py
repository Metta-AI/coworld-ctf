#!/usr/bin/env python3
"""Turn the generated spray-paint-can illustration into the game item master:
data/spraycan.png — the cone weapon's pickup icon and carried marker.
...
CARD CHOICE: the raw art is generated on a flat MEDIUM-GREY card (~rgb 203), not
...
def cut(src: Path) -> tuple[Image.Image, np.ndarray]:
...
def main():
    ...
    held_raw = SRC / "spraycan_held_raw.png"
    if held_raw.exists():
        held, halpha = cut(held_raw)
