...
proc main() =
  ...
  # Open floor with clear line of sight (see the geometry sweep in the session
  # notes: this pose connects, many others are blocked by the slalom).
  let
    midY = MapHeight div 2
    sprayerX = 300
    sprayerY = midY - 90

  # RED carries a can and sprays east at BLUE.
  ...
  # The animation: one frame per FX tick, so the jet's growth is visible. The
  # sprite ids are keyed by stage, so each frame emits its own puff sprites.
  var frames: seq[Image]
  for f in 0 .. PlasmaArcFxTicks:
    let board = renderBoard()
    let shot = cropOf(board, burstCx, sprayerY, BurstW, BurstH)
    frames.add(shot)
    let path = outDir / ("spray-burst-t" & $f & ".png")
    shot.resize(BurstW * 2, BurstH * 2).writeFile(path)
    if f == 0:
      ...
      let spawn = plasmaArcSpawnPoints()[0]
  ...
  # Filmstrip: every frame stacked, so one image shows the whole burst.
