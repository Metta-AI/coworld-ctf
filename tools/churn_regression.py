...
async def guest_churn_loop(session, ws_base, base, tokens, name, hold_s, stop_at):
    # NOTE (2026-08-28, verify-to-close pass on engine 7b0ab22): this lineage
    # does NOT carry the ticket-mint takeover flow that 8/27's churn-stall
    # branch exercised -- freeplaySeatJson() here returns no "ticket" field
    # at all (see src/ctf/server.nim freeplaySeatJson/takeoverRejection at
    # this commit). The takeover WS authenticates with the SEAT'S OWN
    # configured token instead (config.slots[seat].token, populated from the
    # config's top-level "tokens" array) -- the same shape a policy /player
    # join uses. Adapted to match rather than pretend the ticket API exists.
    while time.monotonic() < stop_at:
        ...
        if seat < 0:
            ...
            continue
        token = tokens[seat]
        url = f"{ws_base}/takeover?slot={seat}&token={token}&name={name}"
        try:
