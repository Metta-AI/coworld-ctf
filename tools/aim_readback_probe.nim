...
## fails loudly if any sample disagrees by more than the mapping's error
## bound, i.e. if the readback mapping is wrong.
##
## GV24 NOTE (PR #134, GameVersion "24", task 1217002081645750): POV soldier
## sprites now render at true aim + a deterministic uniform ±14-brad offset
## re-rolled every 12 ticks (AimRenderFuzzBrads=14, AimRenderFuzzWindow=12).
## The mapping error bound is therefore 14 (fuzz) + 8 (half a rotation step of
## quantization) = 22 brads. The old 8-brad bar red-lines on every GV24 replay
## and reads as a mapping break when the mapping is fine. |err| > 22 still
## means the decode chain itself is broken.
##
...
const
  ...
  # GV24: AimRenderFuzzBrads (14) + half a rotation step (8) bounds sprite-
  # derived error when the mapping is correct. Anything past this is a break.
  # (mirrors src/ctf/global.nim AimRenderFuzzBrads, which is not exported)
  AimRenderFuzzBrads = 14
  MaxMappingErr = AimRenderFuzzBrads + SelfRotBrads div 2
...
var
  ...
  aliveMisses = 0
  worstErr = 0
  errHist: array[0 .. 24, int]   # |err| histogram, brads (cap 24 > MaxMappingErr)
...
if worstErr > MaxMappingErr:
  echo &"FAIL: readback disagrees with true aim beyond {MaxMappingErr} brads (fuzz + quantization bound)"
  quit(1)
echo &"OK: readback within {MaxMappingErr} brads (fuzz + quantization bound) of true aim on every sample"
