...
proc generateResponseNode(body: string): JsonNode =
  ...
  let
    ...
    # worse than a slow editor. `validated: false` stays the fast path and is
    # the honest one for "why was this seed rejected": it hands back the raw
    # first draw, rejections included, which is what selection hides.
    gameMap =
