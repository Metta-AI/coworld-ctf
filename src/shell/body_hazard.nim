...
## reads back.
##
## THE ARRIVAL ESTIMATE IS CONSERVATIVE-LATE, NOT EXACT. The planner prices a
## cell by the tick it estimates the cog would arrive there, and that estimate
## is an UPPER bound, never a lower one: it is derived from the length of the
## minimum-COST path (which can be physically longer than the shortest path)
## and from a deliberately derated speed. Paint can therefore be over-priced.
##
## 🚨 That is safe ONLY because this hazard is a PRICE. Over-pricing costs a
## longer route and nothing else. CONVERTING IT TO A FEASIBILITY PRUNE IS
## UNSOUND — a late estimate would prune genuinely reachable ground, and that
## path ends at `hasNoPath` and a cog standing still in a closing zone (round
## 3633). See body_planner.advanceAstar for the full argument and for what a
## sound prune would actually require.
##
## Monotonicity is still load-bearing for the "waiting never helps" half:
## arrival ticks never recede (the zone field's own contract, "MONOTONE by
## construction ... arrival ticks never produce receding paint"). If anyone
## ever makes paint recede, even the price must be re-derived, not re-tuned.
