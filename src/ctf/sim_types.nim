...
import
  ...
  pixie,
  glory
# Every deed/pricing/achievement symbol glory.nim exports (Deed, Tree,
# awardDeed's helpers, XpPer*/LevelThresholds, killDeed, etc.) needs to
# reach sim.nim/roster.nim/broadcast.nim -- everything downstream already
# gets sim_types transitively (the "import sim_types, ...; export
# sim_types, ..." pattern every split file uses), so exporting here once is
# the single place this needs saying, not a re-import per consumer.
export glory
...
const
  ...
  # ── GLORY PORT (GV46) — cosmetic pop tuning, ported verbatim from main's
...
type
  ...
  SimEventKind* = enum
    ...
    # ── GLORY PORT (GV46) ── appended, never inserted, per this enum's own
