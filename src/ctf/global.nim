...
const
  ...
  ## episode — are drawn from a METALLIC RE-BAKE of their own shell art rather
  ## than the stock matte bake, so a spectator knows on sight that the top of the
  ## ladder is on the board (src/ctf/shimmer.nim, docs/COLOR_CONTRACT.md §5). Most
  ## episodes show none.
  ##
  ## The material itself, its sprite KEY space, and the tick->phase schedule all
  ## live in rig_art.nim (applyCogMetal, rigMetalSegSpriteKey, cogMetalPhase);
  ## this module only chooses, per seat, which bake to ask for. There is no
  ## overlay object and no new sprite pool here — the earlier build composited a
  ## separate achromatic sheen sprite on top of the cog at z = y+2, and the whole
  ## point of the rewrite is that a composited decal cannot couple its highlight
  ## to the cog's ORIENTATION, which is the strongest cue the eye uses to call
  ## something metal. Baking the material into art that is ALREADY keyed by aim
  ## step gets that coupling for free.
  ##
  ...
  ##
  ## The heart's paint IS a composited overlay, and the argument that retired the
  ## cog's overlay does not reach it: the case against a decal was that it cannot
  ## couple its highlight to the host's ORIENTATION. The planted heart has no
  ## orientation — it is one static, un-posed sprite parked on a pedestal — so
  ## there is nothing for a re-bake to couple to, and a re-bake would instead
  ## have to re-run the gem's hand-painted master through a material per frame.
  ## The overlay is clipped to that master's own alpha silhouette, which buys the
  ## same "light on THIS surface" read for four sprite pools instead of a
  ## per-team art pipeline.
  HeartShimmerFrames = 24      ## baked sweep positions, one full glide per
                               ## cycle. Sized by STEP: the band travels ~1.1
                               ## gem radii per cycle, so 24 frames move it about
                               ## a map px per step and the glide is smooth,
                               ## where 16 jumped and read as a stutter. (The
                               ## retired cog overlay shared this number; the
                               ## metallic re-bake that replaced it runs its own,
                               ## much smaller CogMetalSweepFrames, because a
                               ## per-aim bake pays 16x for every phase and a
                               ## single 60px overlay does not.)
  HeartShimmerTicksPerFrame = 4 ## 96 ticks = 4s per glide at 24 ticks/s. Slow on
                                ## purpose: the brief is a sheen, not a strobe.
                                ## There is deliberately NO rest phase — the
                                ## bevel is identical in every frame and the band
                                ## never leaves the gem (see
                                ## buildHeartShimmerSprite), so the frame a
                                ## viewer pauses on is never the frame with no
                                ## mark.
  HeartShimmerSpriteBase = 1730 ## planted-heart clearcoat frames, team-major:
                                ## 1730..1825 (4 x HeartShimmerFrames), in the
                                ## gap between the corpses (end 1627) and the
                                ## plasma fx (2002). PER TEAM, because the sheen
                                ## is clipped to the heart's own silhouette and
                                ## the four hand-painted gems are not the same
                                ## shape (heart_blue is a 573x530 master, the
                                ## rest are 458x438). Baked lazily, so a stock
                                ## episode bakes none and a flagged one bakes
                                ## HeartShimmerFrames for one team.
  HeartShimmerObjectBase = 19104 ## one clearcoat overlay per team heart:
                                 ## 19104..19107, in the gap between the identity
                                 ## badges (end 19071) and the impact rings
                                 ## (19120). 19072..19103 in that gap is the hole
                                 ## the retired per-player cog overlay left; it
                                 ## stays a hole rather than being reclaimed, so
                                 ## a replay recorded against either build never
                                 ## sees one feature's object ids reused by the
                                 ## other.
  HeartShimmerTeamStride = 7   ## per-team phase offset in frames. Coprime with
                               ## HeartShimmerFrames, and deliberately different
                               ## from the cog material's CogMetalSeatStride:
                               ## when the flagged policy holds seats on two
                               ## teams the two hearts glide out of step with
                               ## each other, and no heart ever pulses in
                               ## lockstep with the cogs standing next to it.
...
const
  ...
  BoardSpritePools = [
    ...
    ("heart shimmer", HeartShimmerSpriteBase, 4 * HeartShimmerFrames),
...
proc rigMetalSpriteId(
    team: Team, seg: RigSeg, skin: Skin, aimStep, phase: int): int =
  ## Wire id for one metallic-clearcoat SHELL segment (rig_art.applyCogMetal).
  ## Its own logical key space, so the stock pose pools — what every cog in
  ## every unflagged episode draws from — are untouched and unshifted.
  wireSpriteId(rigMetalSegSpriteKey(team, seg, skin, aimStep, phase))
...
proc smoothstep(e0, e1, x: float): float =
  ## Hermite ramp, for the heart clearcoat below. Both halves of the metallic
  ## paint are built out of soft-edged REGIONS (a bevel, a ground crescent, a
  ## band) rather than out of the gaussians they started as, because a region
  ## survives the viewer's box filter and a thin gaussian does not.
  ##
  ## The cog half used to keep a second copy of this nested inside
  ## `buildShimmerSprite`, in this file, with a note to fold the two together
  ## once the cog rework landed. It has landed: that overlay is retired, its
  ## nested copy went with it, and this is now the only ramp in the module.
  ## rig_art.nim's `metalSmoothstep` is deliberately NOT shared with it — it is a
  ## different function, not a duplicate. It carries an `e1 <= e0` guard the
  ## heart does not need, because `-d:metalTune` can drive the cog material's
  ## thresholds past each other at runtime while every threshold here is a
  ## literal, and it is `{.inline.}` inside a per-pixel loop over a much larger
  ## bake. Exporting a module-private two-line ramp across a module boundary to
  ## collapse them would widen rig_art's public surface to save nothing.
...
var heartShells: array[Team, HeartShell]
  ## One basis per team, built on first use and reused across all
  ## HeartShimmerFrames.
...
proc buildHeartShimmerSprite(team: Team, frame: int): seq[uint8] {.measure.} =
  ...
  ## language the cog wears (rig_art.applyCogMetal) — a chrome-ball sky over a
  ## dark ground, a compact catchlight, a gliding glint — re-derived on the gem's
  ## own silhouette instead of on the cog's modelled shell. Analytic, achromatic,
  ## and rasterized at the emission scale, for the same reason the cog material
  ## is: tinting it would put the mark back on the color axis it exists to stay
  ## off.
  ##
  ...
  let
    ...
    # (an identification mark may not have a rest phase, because a paused frame
    # lands on one as often as any other).
    ...
    sweep = -0.55 + 1.10 * (float(frame) + 0.5) / float(HeartShimmerFrames)
...
proc heartShimmerSpriteId(team: Team, frame: int): int =
  ## Team-major, HeartShimmerFrames wide (see HeartShimmerSpriteBase).
  HeartShimmerSpriteBase + ord(team) * HeartShimmerFrames + frame
...
proc heartShimmerFrame(tick: int, team: Team): int =
  ...
  ((tick div HeartShimmerTicksPerFrame) + ord(team) * HeartShimmerTeamStride) mod
    HeartShimmerFrames
...
proc addCogRigObjects(
  ...
  let
    ...
    # METALLIC PAINT, per AGENT: the seats whose stripped policy name is the one
    # league-wide shimmer policy get their SHELL segments (head + shoulders)
    # baked through the clearcoat material instead of the stock matte bake —
    # same silhouette, same labels, different surface. Team-independent on
    # purpose: teammates of a different policy render stock right next to a
    # metal one, and if the flagged policy holds seats on two teams, both
    # shimmer. See rig_art.applyCogMetal for the material and src/ctf/shimmer.nim
    # for why there is at most ONE such policy in a lobby.
    metal = playerShimmers(player.address)
    metalPhase = cogMetalPhase(sim.tickCount, base)
  ...
  proc bakePixels(seg: RigSeg): seq[uint8] =
    ...
    of rsHead:
      if metal:
        rigMetalSegPixels(player.team, rsHead, aimStep, metalPhase,
          renderScale = boardScale, skin = player.skin)
      else:
        rigSegPixels(
          player.team,
          rsHead,
    ...
    of rsArmL, rsArmR:
      # The shoulders take the material but NOT the glint phase: they are ~2x4
      # screen px at true viewer zoom, so nobody can see a glint travel across
      # them, and pinning them to phase 0 cuts the metal pool from 288 keys per
      # flagged team to 128. They still get the material so the shell does not
      # show a seam where a dark metal head meets a stock-bright shoulder.
      if metal:
        rigMetalSegPixels(player.team, seg, aimStep, 0, boardScale)
      else:
  ...
  var segs: seq[tuple[seg: RigSeg, objectId, spriteId, z: int]] = @[
    ...
    (rsHead, RigHeadObjectBase + base,
      (if metal:
         rigMetalSpriteId(player.team, rsHead, player.skin, aimStep, metalPhase)
  ...
  proc armSprite(seg: RigSeg): int =
    ## Phase 0 for the metal shoulders — see bakePixels on why they do not glint.
    if metal: rigMetalSpriteId(player.team, seg, DefaultSkin, aimStep, 0)
    else: rigArmSpriteId(player.team, seg, aimStep, 0)
  segs.add((rsArmL, RigArmObjectBase + base*2 + 0, armSprite(rsArmL), armZ))
  segs.add((rsArmR, RigArmObjectBase + base*2 + 1, armSprite(rsArmR), armZ))
