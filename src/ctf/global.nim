...
proc addRotatingDiamonds(
  ...
  for i in 0 ..< AnimatedDiamonds.len:
    let
      ...
      # Size only: rotatingDiamondPixels returns a COPY of a cached raster,
      # so fetching it eagerly paid that copy per diamond per viewer per
      # frame, when the def-exists checks below drop the send on all but the
      # first. Each branch fetches the pixels only when it will ship them.
    ...
    if paintCount > 0:
      ...
      if defIndex < 0 or spriteDefs[defIndex].label != label:
        let (_, base) = rotatingDiamondPixels(spot.radius, frame, boardScale)
        packet.addBoardSpriteChanged(
          spriteDefs, spriteId, size, size,
          sim.buildPaintedDiamondPixels(i, frame, size, base), label,
          native = boardScale
        )
    elif spriteDefs.spriteDefinitionIndex(spriteId) < 0:
      # The clean stone's def is immutable per frame index, so a viewer
      # that holds it already never touches the pixel cache.
