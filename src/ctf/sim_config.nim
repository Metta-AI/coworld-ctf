...
proc validate(config: GameConfig) =
  ...
  # PATHING: the planner prices the paint DAMAGE surface, which only DEFINES
  # painted ground while zoneDamageByPaint is armed (zonePaintedForDamageAt).
  # Under rect semantics the arrival field describes a picture, not the
  # mechanic, so routing around it would price a hazard the sim never charges
  # — refused loudly, the same "requires X" shape zoneBlocksRevive uses above
