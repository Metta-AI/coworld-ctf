...
suite "player fog-of-war protocol":
  ...
  test "teammates and a teammate-carried flag are never fogged":
    var game = initCtfForTest(defaultGameConfig())
    let viewer = game.addPlayer("red0")
    ...
    var state: PlayerViewerState
    let messages = game.buildPlayerMessages(viewer, state)
    check messages.hasObject(1000 + game.players[mate].joinOrder)
    check messages.hasObject(5010)   # the carried blue flag rides the mate.
