...
#
# THE MARK HAS TWO HALVES and they are found two different ways, on purpose.
#
# COG DETECTION IS BY PIXELS, and it has to be. The cog mark used to be its own
# overlay sprite with its own label, which made it trivial to find; it is now a
# metallic re-bake of the cog's own head art, and that bake keeps the STOCK
# head's label by contract — a label scanner must still read the league #1's cog
# as `player <color>`, or flagging a policy would blind every bot to it. So there
# is no label and no id to look for, and the only honest question left is the one
# a spectator asks: does THIS cog's shell look like metal? `metalOnSeat` answers
# it from the decoded sprite, which also means these tests fail if the material
# ever stops producing a specular — a stricter promise than the old label check
# made.
#
# HEART DETECTION IS BY LABEL, and it can be. The heart's clearcoat is still a
# composited overlay (the planted gem has no orientation for a re-bake to couple
# to), so it owns a label of its own — `<color> flag metal shimmer stage <n>` —
# and it defines no contract label it could shadow. Found black-box by that
# family, then positionally, so a regression that moved it to another pool or
# renamed it fails here.
...
const
  ...
  HeartShimmerLabelPart = " flag metal shimmer stage "
    ...
    ## <n>`), which IS still a labelled overlay and so is still found by label.
    ## Matched on its own distinct family, and on an infix rather than a prefix,
    ## so it can never be confused with the retired per-agent
    ## `metal shimmer stage <n>` overlay whose name it deliberately echoes.
...
proc heartShimmersOnTeam(
  ...
  ## Positional, like `metalOnSeat`: it proves the sheen landed on the right
...
suite "metal shimmer":
  ...
  test "a dead seat does not un-mark its team's heart":
    # The cog material needs a living cog to be baked onto; the TEAM a policy was
    # seated on does not change when its agents die. A heart that dropped its
    # mark mid-firefight and picked it up on respawn would read as a rendering
    # bug. This is also the one check that holds the TWO halves of the mark
    # against each other, which is why it names both detectors: the pixel-based
    # cog probe and the label-based heart probe must disagree here and nowhere
    # else.
    var game = mixedTeamGame()
    setShimmerPolicy("focusfire")                # seat 4, Red, its only seat
    game.players[4].alive = false
    let frame = game.fullFrame()
    check not game.metalOnSeat(frame, 4)         # the cog is gone...
