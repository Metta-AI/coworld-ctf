...
suite "lobby-fill reconnect wedge":
  ...
  test "the same wedge in the serve-forever reset dance (the filed 8-of-16 stall)":
    ...
    # FIXED: nextPlayerSlot() now returns the lowest UNOCCUPIED slot
    # (0, here -- joinOrder 0 vacated while 1..7 stayed seated) instead of
    # the raw count sim.players.len (7, which nothing was ever going to
    # vacate). Slot 0's reconnect resolves to slotIndex 0 (slotOccupied(0)
    # is false) and now matches `join.slotIndex == sim.nextPlayerSlot()`
    # (0 == 0), so admitPendingJoins seats it -- which advances
    # nextPlayerSlot() to 8 and unblocks slots 8..15 in the same pass, all
    # the way to a full roster. This is the reset-dance / 8-of-16 stall,
    # fixed: a mid-stack vacancy no longer wedges every socket behind it.
    check sim.players.len == 16
