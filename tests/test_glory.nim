...
suite "glory: the ace threshold":

  test "the plume is lit at the level that also makes you a bounty":
    # GLORY PORT (GV46): this suite was "glory: the supply drop cannot be
    # farmed" on main. Every OTHER test in it (`supplyDropsFor`, the tap
    # bound, the kit cycle) asserted a farming-resistance property of the
    # supply drop mechanic itself -- `SupplyDropXp`/`SupplyDropMaxPerLife`/
    # `SupplyDropCycle`, none of which exist on this port (v1 BR ships
    # without supply drop; see glory.nim's own header). Dropped, not
    # stubbed: there is no mechanic left for those assertions to describe.
    # This one surviving test asserted something else -- the ACE
    # threshold's own two invariants -- and needed no supply-drop
    # constant, so it's kept.
