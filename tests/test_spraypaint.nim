...
suite "spray cans":
  test "two spray cans spawn walkable in the top half of the side columns":
    ...
    for i in 0 ..< game.sprayPaintSpawns.len:
      check game.sprayPaintSpawns[i].present
      check game.canOccupy(game.sprayPaintSpawns[i].x, game.sprayPaintSpawns[i].y)
      # Spray cans live in the TOP half (quarter height); the shields hold
      # the matching bottom-half spots.
      check abs(game.sprayPaintSpawns[i].y - MapHeight div 4) < 120
      check game.sprayPaintSpawns[i].y < MapHeight div 2
      if i == 0:
        check game.sprayPaintSpawns[i].x < MapWidth div 2
      else:
        check game.sprayPaintSpawns[i].x > MapWidth div 2

  test "pickup has a one-arc limit and respawns after 30 seconds":
    var game = twoTeamGame()
    let spawn = game.sprayPaintSpawns[0]
    game.players[0].placeAtCenter(spawn.x, spawn.y)
    game.tryPickupSprayPaints(0)
    check game.players[0].hasSprayPaint
    check not game.sprayPaintSpawns[0].present
    game.players[1].placeAtCenter(game.sprayPaintSpawns[1].x, game.sprayPaintSpawns[1].y)
    game.tryPickupSprayPaints(1)
    check game.players[1].hasSprayPaint
    game.players[0].placeAtCenter(spawn.x, spawn.y)
    game.tryPickupSprayPaints(0)
    check game.players[0].hasSprayPaint
    game.players[0].placeAtCenter(400, 400)
    game.stepNone(SprayPaintRespawnTicks)
    check game.sprayPaintSpawns[0].present

  test "a carried spray can is lost on death":
    var game = twoTeamGame()
    game.players[0].hasSprayPaint = true
    game.killPlayer(0, 1)
    check not game.players[0].hasSprayPaint
  ...
  test "an arc kills a target in the forward cone and credits the attacker":
    ...
    check game.sprayPaintFlashes.len == 1
  ...
  test "an arc misses behind, outside the cone, and beyond reach":
    ...
    game.players[1].placeAtCenter(
      ax + SprayPaintReach + SprayPaintBodyRadius + 10, ay)
    game.tryFireArc(0)
    check game.players[1].alive
  ...
  test "the cone reaches one body radius past its centerline reach":
    ...
    let
      ax = game.players[0].x + CollisionW div 2
      ay = game.players[0].y + CollisionH div 2
    check SprayPaintBodyRadius == SoldierBodyPx div 2
    game.players[1].placeAtCenter(
      ax + SprayPaintReach + SprayPaintBodyRadius - 2, ay)
    game.tryFireArc(0)
    check not game.players[1].alive
    ...
    game.players[1].placeAtCenter(
      ax + SprayPaintReach + SprayPaintBodyRadius + 4, ay)
    game.tryFireArc(0)
    check game.players[1].alive
  ...
  test "the cone spans 5 squares of reach at an unchanged 14-degree half-angle":
    ...
    let
      ax = game.players[0].x + CollisionW div 2
      ay = game.players[0].y + CollisionH div 2
    check SprayPaintReach == 5 * SprayPaintSquare
    ...
    check SprayPaintMaxWidth * 4 == SprayPaintReach * 2
  ...
  test "spinning after firing never sweeps the cone onto new targets":
    ...
    let
      ...
      ay = game.players[0].y + CollisionH div 2
      northX = ax
      northY = ay - (SprayPaintReach - 2)   # a victim due north, at spray reach.
  ...
  test "same-tick arc fires can kill each other":
    var game = twoTeamGame()
    game.players[0].hasSprayPaint = true
    game.players[1].hasSprayPaint = true
    game.players[0].aimBrads = 0
  ...
  test "a shield carrier survives one spray touch, and only one per burst":
    ...
    check game.players[1].shieldHp == ShieldLayerHp - SprayPaintDamage
  ...
  test "the cone stays live for 5 ticks and catches late entrants":
    ...
    game.tryFireArc(0)
    check game.players[1].alive
    check game.players[0].arcTicksLeft == SprayPaintActiveTicks - 1
  ...
  test "spray can state is in the game hash":
    ...
    game.players[0].hasSprayPaint = false
    game.sprayPaintSpawns[0].present = not game.sprayPaintSpawns[0].present
...
suite "one spray per firing (no divergent trail when the cog turns)":
  ...
  ## sprayPaintRenderPose collapses those onto the newest pose so a moving
  ...
  test "every snapshot of a burst keeps the locked fire aim when the cog turns":
    ...
    check game.sprayPaintFlashes.len == 2
    # Both snapshots carry the LOCKED east aim, not the north swing.
    check game.sprayPaintFlashes[0].aimBrads == 0
    check game.sprayPaintFlashes[1].aimBrads == 0
    # And both render along that same locked aim (the newest pose is still east).
    check game.sprayPaintRenderPose(0).aimBrads == 0
    check game.sprayPaintRenderPose(1).aimBrads == 0
  ...
  test "a burst that turns mid-window paints the locked aim, not the swing":
    ...
    let
      ...
      base = SprayPaintFxObjectBase
      cap = base + MaxPlayers * SprayPaintFxPulses
    var puffs = 0
    for m in messages:
      if m.kind == spkObject and m.objectDef.id >= base and m.objectDef.id < cap:
        ...
        check pcx > cx
        check abs(pcy - cy) < float(SprayPaintFxReach * scale) / 2
    check puffs > 0
...
suite "the damage cone covers the plume the game draws":
  ...
  test "no painted pixel lands past the damage reach":
    ...
    for stage in 0 ..< SprayPaintFxStages:
      for pulse in 0 ..< SprayPaintFxPulses:
        farthest = max(farthest, spraypaintPulseForward(pulse, stage) +
          spraypaintPulseDiameter(pulse, stage) div 2)
    check farthest <= SprayPaintReach
  ...
  test "the plume is sized against the FX span, not the damage reach":
    ...
    check SprayPaintFxReach == 4 * SprayPaintSquare
    check SprayPaintFxMaxWidth == 2 * SprayPaintSquare
    check SprayPaintReach > SprayPaintFxReach
...
suite "spray cone fx wall clipping":
  test "cone pulse discs stop at the first wall along the aim ray":
    ...
    let
      lastPulse = SprayPaintFxPulses - 1
      nearD = float(spraypaintPulseForward(0, 0))
      farD = float(spraypaintPulseForward(lastPulse, 0))
    block search:
    ...
    # must not.
    check messages.hasObject(SprayPaintFxObjectBase + 0)
    check not messages.hasObject(SprayPaintFxObjectBase + lastPulse)
  ...
  test "an unobstructed cone still draws every pulse disc":
    ...
    let messages = game.buildGlobalMessages(state)
    for pulse in 0 ..< SprayPaintFxPulses:
      check messages.hasObject(SprayPaintFxObjectBase + pulse)
