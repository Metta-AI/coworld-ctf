...
const
  ...
  # Under GV35 (elimination deaths uncounted) seed 1 still ends on a
  # capture (Red captures the blue heart, eliminating Blue — the whole
  # blue roster dies on the capture tick, with no deaths-stat bump).
...
suite "broadcast state channel":
  ...
  test "final frame state names the verdict honestly":
    ...
    try:
      ...
      # is pinned to the current recording of the fixture (GameVersion 35,
